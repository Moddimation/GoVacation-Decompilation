...
static const float lnpoly[] = { 1.99999999f, 0.66666948f, 0.39965795f, 0.301003281f };
...
logf (float x)
{
    ...
    else
    {
        ...
        if ((*(_INT32*)&x) <
            0x3F3504F3) // constant for 1/sqrt2  , this check greatly improves accuracy w/only
            x = x + __sqrt2_m1 * x; // minor performance penalty.  without this the 2nd ulp is
                                    // missed 2000 times more frequently
        else
            __lnsqrt2 = 0.0f;

        x    = 1.0f - (2.0f / (x + 1.0f));
        zsq  = x * x;
        y    = (((lnpoly[3] * zsq + lnpoly[2]) * zsq + lnpoly[1]) * zsq + lnpoly[0]) * x;
