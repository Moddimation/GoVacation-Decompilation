...
long double _MSL_CDECL
strtold (const char* _MSL_RESTRICT nptr, char** _MSL_RESTRICT endptr)
{
...
double _MSL_CDECL
strtod (const char* _MSL_RESTRICT str, char** _MSL_RESTRICT end)
{
...
/*- mm 021105 -*/
float _MSL_CDECL
strtof (const char* _MSL_RESTRICT str, char** _MSL_RESTRICT end)
{
 ...
 *the value of &endp must remain unchanged In this case the variable chars_scanned of strtold
 *should be 0 mm  990325 Made to work with input functions passed by pointers mf  990420 turned
 *off k6 calling convention mf  990420 had to provide k6 wrap internal function __strtold as
 *well mm  990817 Deleted include of <string_io.h> mm  990921 Added code to recognize INF, NAN
 *and for hex input mm  990930 Added strtold and general clean up hh  000126 Eliminated the
 *statement that set denormalized values to zero. mm  010503 Added code for thread local storage
 *for lconv mm  010517 Correction to avoid infinite loop when str is "0xy".  IR0105-0110. mm
 *010521 Added _MWMT wrappers cc  010531 Added _GetThreadLocalData's flag mm  011130 Changes for
 *implementation of locale cc  011203 Added _MSL_CDECL for new name mangling JWW 020130 Changed
 *_MWMT to _MSL_THREADSAFE for consistency's sake PMK 020409 Made changes so that strtold will
 *work for little endian machines (X-86) BLC 020829 Fixed implicit conversion errors JWW 020604
 *Added _MSL_LOCALDATA macro to make multithread code easier to read JWW 020917 Added
 *_MSL_RESTRICT to get the restrict type specifier for certain C99 functions ejs 020924 Fixed
 *hexadecimal floating point scanner by making it portable mm  021003 Correction to allow the
 *argument to NAN to contain a radix point cc  021004 Added support for _MSL_C_LOCALE_ONLY BLC
 *021003 Fixed more implicit conversion errors in latest source ejs 021009 Fixed bug in hex
 *float scanning of exponent with leading 0 ejs 021009 Make hex float scanning really
 *independent of 64-bit double size mm  021105 Added strtof() mm  030520 Added C99 wrappers cc
 *040217 Changed _No_Floating_Point to _MSL_FLOATING_POINT cc  040625 Added unaligned accesses
 *bug fix
 */
