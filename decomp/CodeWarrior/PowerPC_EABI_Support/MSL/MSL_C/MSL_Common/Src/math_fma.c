...
fma (double x, double y, double z) _MSL_CANT_THROW
{
    uint64_t     p_hi, p_lo;
    int          shift;
    unsigned int xp;
    ...
    if (x == 0 || ex == 0x7FF00000 || y == 0 || ey == 0x7FF00000 || z == 0 || ez == 0x7FF00000)
        return x * y + z;
    ...
    /* Perform p = x * y
       p will be 118 bits stored in 2 uint64_t's, p_hi:p_lo (only 106 bits are actually
       generated) compute least significant 32 bits of p_lo.  Use p_hi as an accumulator. */
    ...
    if (shift > -106) /* if z is big enough so that it can effect p */
    {
        ...
        if (sp ^ sz) /* subtract */
        {
            ...
            if (p_hi < q_hi)
                ...
                high_sign = 1;
            if (high_sign < 0 || (high_sign == 0 && low_sign < 0) ||
                (high_sign == 0 && low_sign == 0 && xp < xq))
            {
                /* p = q - p */
                if (low_sign > 0)
                    --q_hi;
    ...
    switch (FLT_ROUNDS)
    {
        case 1:    /* to nearest (round to even on tie) */
            if ((p_lo & 0x8000000000000000ULL) &&
                ((p_hi & 1) || (p_lo & 0x7FFFFFFFFFFFFFFFULL) || xp))
                ++p_hi;
            break;
        case 2:    /* towards inf */
            if ((p_lo || xp) && !sp)
...
fmaf (float x, float y, float z) _MSL_CANT_THROW
{
    uint64_t     p_i;
    int          shift;
    unsigned int xp;
    ...
    /* test for cases we don't need extended precision */
    if (x == 0 || ex == 0x7F800000 || y == 0 || ey == 0x7F800000 || z == 0 || ez == 0x7F800000)
        return x * y + z;
    /* all of x, y and z are now either FP_NORMAL or FP_SUBNORMAL
    ...
    {
        case 1:    /* to nearest (round to even on tie) */
            if ((p_i & 0x0000000080000000) &&
                ((p_i & 0x0000000100000000) || (p_i & 0x000000007FFFFFFF) || xp))
