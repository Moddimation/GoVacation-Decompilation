...
fmodf (float x, float y)
{
    ...
    #endif
    quotient >>=
        (31 - exp_shift); // truncates instead of round to nearest(this is correct since we want
                          // largest integer such that (x-Q*y < y).  Round to nearest may
                          // produce one too large. multiply quotient term by 2^^exp_diff , if
                          // exp_diff > 32 this product will be rounded and will not be affected
                          // by the least significant bits of the original y.

    __HI (z) -= exp_diff;
    __HI (y_hi) =
        __HI (y) & 0x7ffff000; // the division of y into two floats(y_lo,y_hi) w/ at most 12
                               // significant bits and quotient in 3 parts each w/12 or fewer
                               // sig bits guarantees that all intermediate products are exact.
                               // This also guarantees that the final subtraction operation when
                               // ordered correctly is exact.  Also note that we use a full 32
                               // bit quotient.  This gives an extra 8 bits of precision for
                               // larger quotients which cannot be represented exactly as an
                               // IEEE float(which has only 24 significant bits). still this
                               // quotient is inadequate for dividend/divisor pairs whose
                               // exponents differ by more than 32
    ...
    z             = fabsf (
        (float)((((y_hi * f_quotient[2] - z) + y_hi * f_quotient[1]) + y_hi * f_quotient[0]) +
