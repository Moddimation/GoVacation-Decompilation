...
unsigned long _MSL_CDECL
strtoul (const char* _MSL_RESTRICT str, char** _MSL_RESTRICT end, int base)
{
    unsigned long value;
...
#if (_MSL_LONGLONG && _MSL_C99) /*- mm 970110 -*/                          /*- mm 030225 -*/
unsigned long long _MSL_CDECL
strtoull (const char* _MSL_RESTRICT str, char** _MSL_RESTRICT end, int base)
{
    ...
    value =
        __strtoull (base, INT_MAX, &__StringRead, (void*)&isc, &count, &negative, &overflow);
...
long _MSL_CDECL
strtol (const char* _MSL_RESTRICT str, char** _MSL_RESTRICT end, int base)
{
    ...
    uvalue =
        __strtoul (base, INT_MAX, &__StringRead, (void*)&isc, &count, &negative, &overflow);
...
strtoll (const char* _MSL_RESTRICT str, char** _MSL_RESTRICT end, int base)
{
...
atoll (const char* str)
{
    return (strtoll (str, NULL, 10));
        ...
        * mm  010423 Correction to prevent leading white space counting in field width in strtoul and
        *strtoull        mm  010506 Correction to mm 010423 to make sure that in the event of failure,
        *chars_scanned is set to zero.        cc  011203 Added _MSL_CDECL for new name mangling        BLC 020829
        *implicit conversion errors fixes, atoi/atol not defined for Palm OS        JWW 020917 Added
        *_MSL_RESTRICT to get the restrict type specifier for certain C99 functions        cc  021209 Added
        *atoll        JWW 030224 Changed __MSL_LONGLONG_SUPPORT__ flag into the new more configurable
        *_MSL_LONGLONG        mm  030225 Added C99 wrappers for LLONG_MAX and ULLONG_MAX        ejs 030723 Fix
        *incorrect typecast in strtoll()
        */
