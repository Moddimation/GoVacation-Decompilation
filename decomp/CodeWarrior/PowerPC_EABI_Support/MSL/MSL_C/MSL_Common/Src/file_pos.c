...
fpos_t _MSL_CDECL
_ftell (FILE* _MSL_RESTRICT file)
{
...
int _MSL_CDECL
fgetpos (FILE* _MSL_RESTRICT file, fpos_t* _MSL_RESTRICT pos)
{
    int crtrgn;                                         /*- mm 020627 -*/
...
_fseek (FILE* file, fpos_t offset, int mode)
{
    ...
    if (file->state.io_state == __neutral)
    {
        if ((pos_proc = file->position_proc) != 0 &&
            (*pos_proc) (file->handle, &offset, mode, file->ref_con))
        {
            set_error (file);
            errno = EFPOS;
            return (-1);
...
rewind (FILE* file)
{
    ...
    file->state.error = 0; /* the standard says we have to do this
                            ...
                            * mm  970623 Change to prevent in buffer seeking with update files since these files can change
                            from reading
                            ...
                            * jz  971105 ( &ma ) Changes to calculate the number of new lines in buffer correctly.  Changes
                            from Mitch Adler and Jay Zipnick
