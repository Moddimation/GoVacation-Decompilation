...
;               /*
               ...
               Execute               ;siz			Datum size               ;ini			Initialized? 0 = filled with zeros, Y = init'ed,
               N = not init'ed               ;end			Virtual address following section
               ;-------------------------------------------------------------------------------------------------
               ;
               ...
               ;naming convention.
               ;
               ;In all cases, any '.' in the name is replaced with a '_'.  Addresses begin with a '_f',
               address               ;after last byte in section begine with '_e' and rom addresses end in a '_rom'.  The
               linker command               ;file allows you to define your own symbols and it allows you to create an alias
               to the               ;automatically generated symbols.
               ;
               ;For example:
...
.extern _rom_copy_info; /* ( __rom_copy_info [] ) An array of all initialized (data, text or
                           mixed) sections */
...
.extern _pic_rom_copy_info; /* ( _pic_rom_copy_info[] ) An array of all initialized (data, text
                               or mixed) sections */
...
.extern _pic_bss_init_info;  /* ( _pic_bss_init_info[] )  An array of all uninitialized data
                                sections */
...
.extern _eti_init_info; /*  ( _eti_init_info[] ) An array of all uninitialized data sections */
...
;                       /*
                       ...
                       ; *	swapped such that code_end < code_start, then segment is internal, otherwise it is
                       external.
                       ; */
...
.extern _pic_eti_init_info; /* ( _pic_eti_init_info[] ) An array of all uninitialized data
                               sections */
...
.extern _f_mwerks_fixup; /* ( unsigned int _f_mwerks_fixup[] ) .mwerks_fixup dest. (RAM) address
                            at beginning */
.extern _e_mwerks_fixup; /* ( unsigned int _e_mwerks_fixup[] ) .mwerks_fixup dest. (RAM) address
                            after end byte */

.extern _ABS_SDA_BASE_;  /* ( char _ABS_SDA_BASE_[] )  same as _SDA_BASE_ but is absolute
                            addressed even with SDA PIC/PID */
.extern _ABS_SDA2_BASE_; /* ( char _ABS_SDA2_BASE_[] ) same as _SDA2_BASE_ but is absolute
                            addressed even with SDA PIC/PID */
