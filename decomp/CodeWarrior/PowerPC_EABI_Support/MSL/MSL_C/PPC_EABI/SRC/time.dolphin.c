/*---------------------------------------------------------------------------*
  Project:  Dolphin MSL time suppert
  File:     time.dolphin.c

  Copyright 2000 Nintendo.  All rights reserved.

  These coded instructions, statements, and computer programs contain
  proprietary information of Nintendo of America Inc. and/or Nintendo
  Company Ltd., and are protected by Federal copyright law.  They may
  not be disclosed to third parties or copied or duplicated in any form,
  in whole or in part, without the prior written consent of Nintendo.

  $Log: time.dolphin.c,v $
  Revision 1.6  2005/12/12 23:05:32  cgalvan
  Changing Copyright to Freescale Corp.

  Revision 1.4.6.1  2005/12/12 20:17:34  cgalvan
  Changing Copyright to Freescale Corp.

  Revision 1.4  2003/06/25 20:55:12  ceciliar
  added changes done on msl_core_1

  Revision 1.3  2002/08/05 19:25:15  ceciliar
  added include to time_api.h

  Revision 1.2.2.1  2002/07/30 20:49:28  ceciliar
  Added check include <time_api.h>

  Revision 1.2  2001/10/05 17:34:17  swells
  Initial commit

  Revision 1.1.2.1  2000/10/30 22:27:33  swells
  Updates for R1.0

  $NoKeywords: $
 *---------------------------------------------------------------------------*/
/*  MSL  Version 2.1.2  1997 May  */

// Include your OS dependent include files
#include <time.h>
#include <time_api.h> /*- cc 020730 -*/

// This has a dependency on the Dolphin OS existing
// OS.h is needed to get OS_TIMER_CLOCK and OSTicksToSeconds.
// swells on 08-22-00
#include <dolphin/os.h>

#define BIAS ((100 * 365LU + 24) * 86400) // Dolphin counts since midnight, 1/1/2000

// __get_clock now returns the number of ticks since startup
// swells on 06-23-2003
clock_t
__get_clock (void)
{
    return (clock_t)OSGetTick();
}

// __get_time returns the current calendar time in seconds since midnight, 1/1/1900
time_t
__get_time (void)
{
    return (time_t)(OSTicksToSeconds (OSGetTime())) + BIAS;
}

// Dolphin does not support time zone and just returns 0
int
__to_gm_time (time_t* time)
{
#pragma unused(time)
    return 0;
}

/*
 * New for 2.4
 * Check if Daylight savings time is active
 * Currently the OS can override this if need be.
 * This always returns false
 */
int
__isdst (void)
{
    return 0;
}
