...
_MSL_IMP_EXP_C double _MSL_MATH_CDECL
nexttoward (double x, long double y) _MSL_CANT_THROW
{
    int            increase;
    int            positive;
...
cbrt (double x) _MSL_CANT_THROW
{
    ...
    y = (0.1950252994681516 +
         (8.93386164028292 + (34.95109317740758 + 14.90468137136715 * x) * x) * x) /
...
_MSL_IMP_EXP_C int _MSL_MATH_CDECL
ilogb (double x)
{
    if (x == 0.0)
        return (FP_ILOGB0);
...
_MSL_IMP_EXP_C double _MSL_MATH_CDECL
tgamma (double x) _MSL_CANT_THROW
{
    return ((double)tgammal (x));
}

_MSL_IMP_EXP_C double _MSL_MATH_CDECL
nextafter (double x, double y) _MSL_CANT_THROW
{
    return nexttoward (x, (long double)y);
                    ...
                    *_MSL_FLOATING_POINT flag check                    cc  040518 File has been split                    cc  041203 Moving back nextafter
                    *from math_sun.c                    cc  060303 Adding erf from sun
                    */
