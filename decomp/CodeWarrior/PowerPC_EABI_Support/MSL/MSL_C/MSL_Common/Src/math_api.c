...
unsigned _MSL_MATH_CDECL
__msl_generic_count_bits32 (_UINT32 x) _MSL_CANT_THROW
{
    x -= (x >> 1) & 0x55555555;
...
unsigned int _MSL_MATH_CDECL
__msl_generic_count_leading_zero32 (_UINT32 x) _MSL_CANT_THROW
{
    x |= (x >> 1);
    x |= (x >> 2);
    x |= (x >> 4);
...
unsigned int _MSL_MATH_CDECL
__msl_generic_count_bits64 (_UINT64 x) _MSL_CANT_THROW
{
...
unsigned int _MSL_MATH_CDECL
__msl_generic_count_leading_zero64 (_UINT64 x) _MSL_CANT_THROW
{
    x |= (x >> 1);
    x |= (x >> 2);
    x |= (x >> 4);
...
short
__msl_relation (double x, double y)
{
    if (isnan (x) || isnan (y))
        return (3); /* unordered    */
    else if (x > y)
    ...
    /*#if (_MSL_C99 || _MSL_USES_SUN_MATH_LIB)*/
        ...
        #if _MSL_C99 || _MSL_USES_SUN_MATH_LIB
int _MSL_MATH_CDECL
__signbitf (float x) _MSL_CANT_THROW
{
    return ((_INT32)(__HI (x) & 0x80000000));
}

int _MSL_MATH_CDECL
__fpclassifyf (float x) _MSL_CANT_THROW
{
...
int _MSL_MATH_CDECL
__signbitd (double x) _MSL_CANT_THROW
{
    return ((_INT32)(__UHI (x) & 0x80000000));
}

int _MSL_MATH_CDECL
__fpclassifyd (double x) _MSL_CANT_THROW
{
