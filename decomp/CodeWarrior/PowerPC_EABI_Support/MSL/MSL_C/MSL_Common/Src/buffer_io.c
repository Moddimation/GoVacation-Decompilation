...
#if (__dest_os == __mac_os || __dest_os == __mips_bare) && defined(__MWERKS__)
...
int _MSL_CDECL
setvbuf (FILE* _MSL_RESTRICT file, char* _MSL_RESTRICT buff, int mode, size_t size)
{
...
void _MSL_CDECL
setbuf (FILE* _MSL_RESTRICT file, char* _MSL_RESTRICT buff)
{
    if (buff)
        ...
        setvbuf (file, 0, _IONBF, 0);
                                                       ...
                                                       * mm  021008 Deleted _emb_68k wrapper from decision to use Mac newline conversion, _emb_68k
                                                       uses Windows terminals
