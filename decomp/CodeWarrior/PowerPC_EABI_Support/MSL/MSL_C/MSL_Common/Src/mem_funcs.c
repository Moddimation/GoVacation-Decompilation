...
void _MSL_CDECL
__copy_mem (void* _MSL_RESTRICT dst, const void* src, unsigned long n)
{
    const char*         p;
    ...
    for (p = (const char*)src, q = (char*)dst, n++; --n;) *q++ = *p++;
...
void _MSL_CDECL
__copy_longs_aligned (void* _MSL_RESTRICT dst, const void* src, unsigned long n)
{
#if (defined(__MOTO__) || defined(__MIPS__))
    ...
    if (i)
    {
        ...
        do deref_auto_inc (cpd) = deref_auto_inc (cps);
        while (--i);
    }
    ...
    for (i = n >> 2; i; i--) { deref_auto_inc (lpd) = deref_auto_inc (lps); }
#else
    i = n >> 5;
    ...
    if (i)
        do deref_auto_inc (lpd) = deref_auto_inc (lps);
        while (--i);
#endif
...
__copy_longs_rev_aligned (void* dst, const void* src, unsigned long n)
{
    ...
    // now the 'long' copy. Need to update long pointers
    lps = ((unsigned long*)cps);
    lpd = ((unsigned long*)cpd);

#ifdef _ENTERPRISE_C_
    for (i = n >> 2; i; i--) { *--lpd = *--lps; }
#else
    i = n >> 5;
    ...
    // go back to 'char' copy. Need to update char pointers
...
__fill_mem (void* dst, int val, unsigned long n)
{
    ...
    #ifdef _ENTERPRISE_C_
        for (i = n >> 2; i; i--) { deref_auto_inc (lpd) = v; }
    #else
        i = n >> 5;
