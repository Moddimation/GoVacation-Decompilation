...
size_t _MSL_CDECL
wcsftime (wchar_t* _MSL_RESTRICT         str,
          size_t                         max_size,
          const wchar_t* _MSL_RESTRICT   format_str,
          const struct tm* _MSL_RESTRICT timeptr)
{
    ...
    while (*format_ptr)
    {
        ...
        switch (*++format_ptr)
        {
            ...
            case L'I':
                num_chars =
                    wemit (str, 2, &space_remaining, L"%.2d", (n = tm.tm_hour % 12) ? n : 12);
                break;
            ...
            case L'p':
                strcpy (temp_string,
                        __MSL_extract_name (current_locale_ptr->time_cmpt_ptr->am_pm,
                                            tm.tm_hour < 12 ? 0 : 1));
            ...
            case L'x':
                ...
                if (char_cnt == -1)                                         /*- mm 040519 -*/
                    return (0);                                             /*- mm 040519 -*/
                num_chars =
                    wcsftime (str, space_remaining + 1, wtemp_string, &tm); /*- mm 040519 -*/
                                    ...
                                    /*
                                                case L'Z':
                                                    if (*current_locale_ptr->time_cmpt_ptr->TimeZone == '\0')
                                                    {
                                                        ...
                                                        num_chars = wemit(str,
                                       strlen(current_locale_ptr->time_cmpt_ptr->TimeZone), &space_remaining, L"%s",
    ...
    *str = 0;
    if (max_size < chars_written)
        ...
        return (chars_written);
                                     ...
                                     * mm  990322 Allow for the fact that the count wsprintf includes the terminating null char.                                     See
                                     * C9x                                     mm  990322 Make two char string constants into wchar_t constants.                                     mm  990331 Correct
                                     * length to allow for the fact that swfprintf takes %s to designate a char and not wide char
                                     * str                                     mm  990809 Added include of wtime.h                                     cc  000326 Removed dest_os to be_os                                     mm  010327
                                     * Commented wasctime and wctime out since they are not defined in C99 and wasctime tampered
                                     * with gmtime's threadlocal data                                     mm  010421 Corrected setting of tm_isdst                                     cc  010510 Changed
                                     * __add to __msl_add                                     cc  010510 Changed __div to __msl_div                                     cc  010510 Changed __ladd to
                                     * __msl_ladd                                     cc  010510 Changed __lmul to __msl_lmul                                     cc  010510 Changed __mod to __msl_mod                                     mm
                                     * 010521 Deleted #include of ThreadLocalData.h                                     JWW 010918 Use time_api.h to get clock and time
                                     * APIs                                     cc  011203 Added _MSL_CDECL for new name mangling                                     cc  010210 Added changes from
                                     * msl_core_1 for e68k team                                     JWW 020917 Added _MSL_RESTRICT to get the restrict type specifier
                                     * for certain C99 functions                                     mm  021220 Many changes to wcsftime() to bring into C99
                                     * conformance.                                     mm  030103 Corrected case of case label in wcsftime()                                     cc  030131 Added cast to
                                     * const char *                                     JWW 030224 Changed __NO_WIDE_CHAR flag into the new more configurable
                                     * _MSL_WIDE_CHAR                                     mm  030304 Added _MSL_C99 wrapper                                     cc  040204 The _No_Time_OS_Support flag has
                                     * been deprecated and replaced with _MSL_OS_TIME_SUPPORT                                     cc  040217 Changed _No_Floating_Point
                                     * to _MSL_FLOATING_POINT                                     mm  040519 Changed code to deal with %x and %X                                     cc  060307 Changed %Z
                                     * to be that same as %z
                                     */
