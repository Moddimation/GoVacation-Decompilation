...
__pool_alloc (mem_pool_obj* pool_obj, mem_size size)
{
    ...
    /*if (size = 0 || size > ULONG_MAX - (sizeof(tag_word) + alignment)) bkoz 4 malloc(0), fixed
     * in new.cp*/
    #ifndef __ALTIVEC__
...
__pool_alloc_clear (mem_pool_obj* pool_obj, mem_size size)
{
    ...
    for (size = (size / sizeof (long)) + 1, p = ptr; --size;) *p++ = 0;

        #else

    for (size = (size / sizeof (long)) + 1, p = ptr - 1; --size;) *++p = 0;
    ...
    #endif                                                 /* __ALTIVEC__ */
{
    ...
    #ifdef __ALTIVEC__
    new_size =
        (new_size + pool_obj->options.block_alignment) & ‾pool_obj->options.block_alignment;
    #else
    ...
    else if (split_size < block_overhead)
        return (block_size);
    ...
    #endif                                                 /* __ALTIVEC__ */
{
    ...
    return (split_block (free_list, block, new_size));
        ...
        * hh  990227 Logic rewritten and moved into alloc.c.  Same algorithm exists in alloc.c as
        *"Design 1".        BOBC990302 AltiVec requires larger alignment (in a seperate pool).        hh  990504
        *Corrected prototypes for split_block and grow_block        vss 990808 Empty file is non-standard, but
        *we're going to ignore it anyway        cc  020109 Added _MSL_CDECL work        hh  020214 Renamed
        *_MSL_PRO4_MALLOC to _MSL_CLASSIC_MALLOC        JWW 040401 Changed _No_Alloc_OS_Support to
        *_MSL_OS_ALLOC_SUPPORT        cc  040629 Changed #ifdef __ALTIVEC__ to #ifndef __ALTIVEC
        */
