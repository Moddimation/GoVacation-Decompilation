...
_MSL_INLINE void
__init_critical_regions (void) _MSL_CANT_THROW
{
    int i;

    for (i = 0; i < num_critical_regions; i++) pthread_mutex_init (&__cs[i], 0);
...
_MSL_INLINE void
__kill_critical_regions (void) _MSL_CANT_THROW
{
    int i;
    ...
    for (i = 0; i < num_critical_regions; i++) pthread_mutex_destroy (&__cs[i]);
}

_MSL_INLINE void
__begin_critical_region (int region) _MSL_CANT_THROW
{
    pthread_mutex_lock (&__cs[region]);
}

_MSL_INLINE void
__end_critical_region (int region) _MSL_CANT_THROW
{
