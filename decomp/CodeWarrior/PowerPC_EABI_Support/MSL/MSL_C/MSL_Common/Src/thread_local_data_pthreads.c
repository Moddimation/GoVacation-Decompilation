...
static int
_InitializeThreadDataIndex (void)
{
    ...
    /* Call free() on the thread data when a thread terminates and it has local data associated
     * with it */
...
_InitializeThreadData (void)
{
    ...
    if (tld == NULL)
    {
        ...
        memcpy ((void*)&tld->_current_locale.next_locale,
                (void*)&_current_locale.next_locale,
...
int _MSL_CDECL
__msl_InitializeMainThreadData (void)
{
...
void _MSL_CDECL
__msl_DisposeAllThreadData (void)
{
    if (_msl_local_data_key != 0)
    {
...
_ThreadLocalData* _MSL_CDECL
__msl_GetThreadLocalData (void)
{
    _ThreadLocalData* tld;
    ...
    return tld;
            ...
            * JWW 040920 Call free() instead of custom _tld_destructor() since free should never be
            * unloaded
            */
