 ...
 *
 * $Date: 2009/03/10 20:59:57 $
 * $Revision: 1.39.2.2.2.1 $
 */
...
size_t _MSL_CDECL
fread (void* _MSL_RESTRICT ptr, size_t memb_size, size_t num_memb, FILE* _MSL_RESTRICT file)
{
    size_t retval;
...
size_t _MSL_CDECL
fwrite (const void* _MSL_RESTRICT ptr,
        size_t                    memb_size,
        size_t                    num_memb,
...
__fwrite (const void* ptr, size_t memb_size, size_t num_memb, FILE* file)
{
    ...
    if (file->state.io_state == __neutral)
        ...
        {
    #if !defined(__BEOS__) &&                                                                  \
        _MSL_OS_DISK_FILE_SUPPORT /* we do O_APPEND on open */   /*- mm 970708 -*/
    ...
    return (bytes_written / memb_size);                          /*- mm 031230 -*/
                                                                  ...
                                                                  *_MSL_WIDE_CHAR                                                                  mm  030314 Added _MSL_C99 wrappers for fwide                                                                  mm  031230 Changes to ensure that
                                                                  *fwrite() returns correct value after an error. MTWX10103                                                                  mm  040128 Completed 031230 MTWX10103
                                                                  *change, thanks to Jim Walker                                                                  cc  040217 Changed _No_Disk_File_OS_Support to
                                                                  *_MSL_OS_DISK_FILE_SUPPORT                                                                  JWW 040406 Changed _Unbuffered_IO to _MSL_BUFFERED_IO                                                                  mm  040426
                                                                  *Change to avoid nested critical regions.
                                                                  */
