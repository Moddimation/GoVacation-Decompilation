...
size_t _MSL_CDECL
_MSL_ALLOCATE_SIZE (void* ptr)
{
    return __sys_pointer_size (ptr);
}

void* _MSL_CDECL
_MSL_ALLOCATE (size_t size_requested, size_t* size_received)
{
    void* result = 0;
    if (size_requested)
...
int _MSL_CDECL
_MSL_ALLOCATE_RESIZE (void* ptr, size_t size_requested, size_t* size_received)
{
...
int _MSL_CDECL
_MSL_ALLOCATE_EXPAND (void* ptr, size_t min_size, size_t preferred_size, size_t* size_received)
{
    int     result = 0;
    size_t  sr_backup;
 ...
 *
 ***************************************************************************/
extern int _MSL_CDECL
init_alloc (void* pool_ptr, mem_size size)
{
    if (!initialized)
    {
        /***************************************************************************
...
static const size_t allocate_mask =
    _MSL_ALIGNMENT == 1 ? (size_t)(-1) << (CHAR_BIT * sizeof (size_t) - 1) : 1;

    #define align_up(size)                                                                     \
        ((__typeof__ (size))((size_t)size + (_MSL_ALIGNMENT - 1) & ‾(_MSL_ALIGNMENT - 1)))
    ...
    init_default_heap (void)
{
    if (_MSL_HEAP_SIZE >= align_up (3 * sizeof (void*)) + align_up (sizeof (void*)))
    {
        ...
        if (_MSL_ALIGNMENT == 1)
            assert ((((size_t)__heap + _MSL_HEAP_SIZE) &
...
/*
    unsigned char** insert(unsigned char** e, unsigned char* value, unsigned char** p, size_t*
   nblocks)
...
static
    #else
unsigned char** insert (unsigned char** e,
                        unsigned char*  value,
                        unsigned char** p,
                        size_t*         nblocks);
    #endif
    unsigned char**
    insert (unsigned char** e, unsigned char* value, unsigned char** p, size_t* nblocks)
{
    unsigned char** n = 0;
...
void erase (unsigned char** e, unsigned char** i, size_t* nblocks);
    #endif
    void
    erase (unsigned char** e, unsigned char** i, size_t* nblocks)
{
    if (!is_allocated (*(e + 1)))
    {
        for (++e; i > e; --i) *i = *(i - 1);
...
void erase2 (unsigned char** e, unsigned char** i, size_t* nblocks);
    #endif
    void
    erase2 (unsigned char** e, unsigned char** i, size_t* nblocks)
{
    if (!is_allocated (*(e + 1)))
    {
        for (e += 2; i > e; --i) *i = *(i - 2);
...
/*
    static unsigned char** lower_bound(unsigned char** first, unsigned char** last, unsigned
   char* value)

    lower_bound is a binary searching function lifted from MSL C++.  Care has been taken to use
   > as the comparison instead of < since the array is stored in reverse order.  This function
   searches for value in the range [first, last) and returns an unsigned char** pointing to that
   value.
*/
...
unsigned char** lower_bound (unsigned char** first, unsigned char** last, unsigned char* value);
    #endif
    unsigned char**
    lower_bound (unsigned char** first, unsigned char** last, unsigned char* value)
{
...
_MSL_ALLOCATE (size_t size_requested, size_t* size_received)
{
    ...
    if (!defaultheapinitialized)
        init_default_heap();
    if (size_requested && size_requested < (size_t)(-1) - _MSL_ALIGNMENT &&
...
int _MSL_CDECL
_MSL_ALLOCATE_RESIZE (void* vp, size_t size_requested, size_t* size_received)
{
    unsigned char*  pc              = (unsigned char*)vp;
...
/*
    int _MSL_ALLOCATE_EXPAND(void* vp, size_t min_size, size_t preferred_size, size_t*
   size_received)
...
int _MSL_CDECL
_MSL_ALLOCATE_EXPAND (void* vp, size_t min_size, size_t preferred_size, size_t* size_received)
{
    int     result = 0;
    size_t  sr_backup;
    size_t* srp = &sr_backup;
...
#elif !defined(_MSL_OS_DIRECT_MALLOC) &&                                                       \
    !(defined(_MSL_PRO4_MALLOC) || defined(_MSL_CLASSIC_MALLOC))
    #pragma mark -
...
/*	All returned memory will be aligned as indicated. */
    ...
    //	You can reserve a pool for a small range of sizes.  The use of these pools can be
    disabled
    ...
    //  1. Each pool will handle approx. 4000 bytes worth of requests before asking for more
    memory.
...
static const mem_size max_fix_pool_size =
    76; /* Need the highest poolsize elsewhere, so kludge it up like this */
static const mem_size fix_pool_sizes[] = {
    4, 12, 28, 44, 60, max_fix_pool_size
};      /* Client space for each pool */
        #else
static const mem_size max_fix_pool_size =
    68; /* Need the highest poolsize elsewhere, so kludge it up like this */
static const mem_size fix_pool_sizes[] = {
    4, 12, 20, 36, 52, max_fix_pool_size
};      /* Client space for each pool */
...
static const mem_size SubBlock_min_size =
    SubBlock_overhead + max_fix_pool_size + 1 + (alignment - 1) & ‾(alignment - 1);
    #else
static const mem_size SubBlock_min_size =
    SubBlock_overhead + 3 * sizeof (mem_size) + (alignment - 1) & ‾(alignment - 1);
    #endif                                      /* _MSL_USE_FIX_MALLOC_POOLS */
...
Block_report (Block* ths, int verbose)
{
    ...
    i   = 1;
    if (verbose)
        printf ("\tsize_ = %d, max_size = %d¥n",
                Block_size (ths),
                Block_max_possible_size (ths));
    if (Block_size (ths) > 1042 * 1024)
    {
        printf ("\t**ERROR** Block size suspiciously large %d¥n", Block_size (ths));
        exit (1);
    }
    prev_free = 0;
    do {
        ...
        {
            if (verbose)
                printf ("\tSubBlock %d at %p¥n", i, st);
            if (prev_free)
            {
                printf ("\t**ERROR** SubBlock failed to merge with previous¥n");
                exit (1);
            }
    ...
    if (sb != 0)
    {
        do {
            ...
            {
                printf ("\t**ERROR** SubBlock size suspiciously large %d¥n",
                        SubBlock_size (sb));
                exit (1);
            }
            sb = sb->next_;
...
{
    if (verbose)
        printf ("\t¥tsize_ = %d, bp_ = %p¥n", SubBlock_size (ths), SubBlock_block (ths));
    if (SubBlock_size (ths) > 1042 * 1024)
    {
        printf ("\t**ERROR** SubBlock size suspiciously large %d¥n", SubBlock_size (ths));
        exit (1);
    }
}
...
static void FixBlock_construct (FixBlock*    ths,
                                FixBlock*    prev,
                                FixBlock*    next,
                                mem_size     index,
                                FixSubBlock* chunk,
...
static void
FixBlock_construct (FixBlock*    ths,
                    FixBlock*    prev,
                    FixBlock*    next,
                    mem_size     index,
                    FixSubBlock* chunk,
...
void _MSL_CDECL
__init_pool_obj (__mem_pool* pool_obj)
{
...
allocate_from_fixed_pools (__mem_pool_obj* pool_obj, mem_size size, size_t* client_received)
{
    ...
    if (fs->head_ == 0 || fs->head_->start_ == 0)
    {
        ...
        while (n >= 10)
        {
            ...
            newblock =
                (char*)soft_allocate_from_var_pools (pool_obj, size_requested, &size_has);
            if (newblock != 0)
                break;
        ...
        if (newblock == 0)
        {
            ...
            while (1)
            {
                ...
                newblock =
                    (char*)allocate_from_var_pools (pool_obj, size_requested, &cr_backup);
                if (newblock != 0)
                    break;
                n = cr_backup /
        ...
        FixBlock_construct ((FixBlock*)newblock,
                            fs->tail_,
                            fs->head_,
                            i,
...
__pool_allocate_resize (__mem_pool* pool,
                        void*       ptr,
                        ...
                        size_t*     size_received)
{
    ...
    {
    #endif                                      /* _MSL_USE_FIX_MALLOC_POOLS */
        if (size_requested > MAX_MEM_SIZE - (alignment + SubBlock_overhead + Block_overhead +
                                             alignment + SubBlock_min_size))
            size_requested = MAX_MEM_SIZE - (alignment + SubBlock_overhead + Block_overhead +
                                             alignment + SubBlock_min_size);
