...
extern _MSL_INLINE float _inv_sqrtf (float x);

#if __USING_INTRINSIC_SQRT__

    #define __HAS_INV_SQRTF 1

extern _MSL_INLINE float
_inv_sqrtf (float x)
{
    if (x > 0.0f)
    {
...
_inv_sqrtf (float x)
{
    ...
    */
    ...
    {
        (*(_UINT32*)&scaled_x) =
            0x3f000000 + ((*(_INT32*)&x) & 0x007fffffU); /* scaled_x in [. 5,1.0) */
...
sqrt (double x)
{
    ...
    */
        ...
        /*	Zen has optimized sqrt routines */
            ...
            #define __HAS_SQRTF 1
extern _MSL_INLINE float sqrtf (float);
...
extern _MSL_INLINE float _MSL_MATH_CDECL
fabsf (float x)
{
    #if defined(__MWERKS__) && (__option(e500_floatingpoint) || __option(e500v2_floatingpoint))
