//
//	ExceptionPPC.cp		-	Exception Handling Runtime Support for CodeWarriorｪ (PowerPC)
//
//  Copyright ｩ 1995-2006 Freescale Semiconductor, Inc.  All rights reserved.
//  $Date: 2006/02/18 00:03:31 $
//  $Revision: 1.7 $
//
//
...
#endif
#if __PPCe500__
    #warning GPR throwcontext is a union, as vec64bit is not as functional as needed to work here
...
/************************************************************************/
int
__register_fragment (char* code_start,
                     char* code_end,
                     char* data_start,
                     char* data_end,
...
ExPPC_FindExceptionRecord (char* returnaddr, MWExceptionInfo* info)
{
    ...
    info->exception_record =
        ETI_GetDirectStore (p->eti_field)
    ...
    if (ET_IsLargeTable (info->exception_record->et_field))
    {
        ...
        for (erl = etl->ranges; erl->start != 0; erl++)
        {
            unsigned long range_end =
                erl->start + (erl->size * 4); // large range is 256K (64K * 4) ... (+4?)
...
ExPPC_PopR31 (char* SP, MWExceptionInfo* info)
{
    ...
    #if !__PPC_EABI__
    if (saved_FPRs & 1)
        GPR_save_area -= 2; // 8-byte gap if # saved FPRs is odd
    #endif
#else
    //  find saved GPRs
    saved_GPRs    = ET_GetSavedGPRs (info->exception_record->et_field);
    ...
    {
        return (GPR_save_area[-2]); //	R31 is last register saved, has highest address
                                    //  This is a 64 bit register so -2
    }
#endif
...
ExPPC_NextAction (ActionIterator* iter)
{
    ...
    {
        if (iter->info.action_pointer == 0 ||
            ((action = ((ex_destroylocal*)iter->info.action_pointer)->action) &
             EXACTION_ENDBIT) != 0)
...
//	for(i=32-saved_GPRs, j=0;i<32;++i,++j) context->GPR[i]=GPR_save_area[j];
    ...
    if (ET_HasElfVector (info->exception_record->et_field))
    {
    #warning This will result in block copy rather then elf code, fix sometime
        for (i = 32 - saved_GPRs, j = 0; i < 32; ++i, ++j)
            context->GPR[i] = Vector_save_area[j];
    }
    else
    {
        for (i = 32 - saved_GPRs, j = 0; i < 32; ++i, ++j)
        {
            ...
            context->GPR[i].l[0] = GPR_save_area[j];
            context->GPR[i].l[1] = 0;
    #else
            context->GPR[i].l[1] = GPR_save_area[j];
            context->GPR[i].l[0] = 0;
    #endif
        }
    }
    ...
    if (ET_HasVectorInfo (info->exception_record->et_field))
    {
        ...
        VR_save_area    = (MWEVector128*)((unsigned int)VR_save_area &
                                       0xFFFFFFF0); // align to 16-byte boundary
...
ExPPC_DestroyLocalCond (ThrowContext* context, const ex_destroylocalcond* ex)
{
    ...
    int cond = ex_destroylocalcond_GetRegCond (ex->dlc_field)
                   ? (local_cond_type)context->GPR[ex->cond].c[0]
                   : *(local_cond_type*)(context->FP + ex->cond);
    #else
    int cond = ex_destroylocalcond_GetRegCond (ex->dlc_field)
                   ? (local_cond_type)context->GPR[ex->cond].c[7]
                   : *(local_cond_type*)(context->FP + ex->cond);
    #endif
#else
    int cond = ex_destroylocalcond_GetRegCond (ex->dlc_field)
...
ExPPC_DestroyLocalPointer (ThrowContext* context, const ex_destroylocalpointer* ex)
{
    ...
    void* pointer = ex_destroylocalpointer_GetRegPointer (ex->dlp_field)
                        ? (void*)context->GPR[ex->pointer].l[0]
                        : *(void**)(context->FP + ex->pointer);
    #else
    void* pointer = ex_destroylocalpointer_GetRegPointer (ex->dlp_field)
                        ? (void*)context->GPR[ex->pointer].l[1]
                        : *(void**)(context->FP + ex->pointer);
    #endif
#else
    void* pointer = ex_destroylocalpointer_GetRegPointer (ex->dlp_field)
...
ExPPC_DestroyMember (ThrowContext* context, const ex_destroymember* ex)
{
    ...
    char* objectptr = ex_destroymember_GetRegPointer (ex->dm_field)
                          ? (char*)context->GPR[ex->objectptr].l[0]
                          : *(char**)(context->FP + ex->objectptr);
    #else
    char* objectptr = ex_destroymember_GetRegPointer (ex->dm_field)
                          ? (char*)context->GPR[ex->objectptr].l[1]
                          : *(char**)(context->FP + ex->objectptr);
    #endif
#else
    char* objectptr = ex_destroymember_GetRegPointer (ex->dm_field)
...
ExPPC_DestroyMemberCond (ThrowContext* context, const ex_destroymembercond* ex)
{
    ...
    int   cond      = ex_destroymembercond_GetRegCond (ex->dmc_field)
                          ? (vbase_ctor_arg_type)context->GPR[ex->cond].l[0]
    ...
    char* objectptr = ex_destroymembercond_GetRegPointer (ex->dmc_field)
                          ...
                          : *(char**)(context->FP + ex->objectptr);
    int   cond      = ex_destroymembercond_GetRegCond (ex->dmc_field)
                          ? (vbase_ctor_arg_type)context->GPR[ex->cond].l[1]
                          : *(vbase_ctor_arg_type*)(context->FP + ex->cond);
    #endif
#else
    char* objectptr = ex_destroymembercond_GetRegPointer (ex->dmc_field)
    ...
    for (;;)
    {
        if (info->action_pointer == 0)
        {
            ...
            context->FP = (ET_GetHasFramePtr (info->exception_record->et_field))
                              ? (char*)context->GPR[31].c[0]
                              : context->SP;
    #else
            context->FP = (ET_GetHasFramePtr (info->exception_record->et_field))
                              ? (char*)context->GPR[31].c[7]
                              : context->SP;
    #endif
#else
            context->FP = (ET_GetHasFramePtr (info->exception_record->et_field))
        ...
        switch (action & EXACTION_MASK)
        {
            case EXACTION_BRANCH:
                info->action_pointer = ((char*)info->exception_record) +
                                       ((ex_branch*)info->action_pointer)->target;
                break;
            ...
            case EXACTION_DESTROYLOCALPOINTER:
                ExPPC_DestroyLocalPointer (context,
                                           (ex_destroylocalpointer*)info->action_pointer);
            ...
            case EXACTION_DESTROYMEMBERARRAY:
                ExPPC_DestroyMemberArray (context,
                                          (ex_destroymemberarray*)info->action_pointer);
            ...
            case EXACTION_ACTIVECATCHBLOCK:
                {
                    ...
                    catchinfo =
                        (CatchInfo*)(context->FP +
                                     ((ex_activecatchblock*)info->action_pointer)->cinfo_ref);
                    if (catchinfo->dtor)
                    {
...
ExPPC_LongJump (register ThrowContext* context, register void* newRTOC, register void* newPC)
{
    ...
    nofralloc
        ...
        mr       r8,
        newPC mr RTOC,
        newRTOC                    //	restore RTOC
            lwz r0,
        context->CR                //	restore CR
            mtcrf 255,
        r0
    ...
    #if __VEC__
            lwz r5,
        context->vrsave bl __setLR // set LR to address of setLR
            __setLR :

      oris r5,
        r5,
        0x8000                     // indicate vr0 also in-use
        mtvrsave r5 clrlwi r5,
        r5,
        20                         // clear left 20 bits
        cntlzw r5,
        r5                         // find first nonvolatile VR used
            slwi r5,
        r5,
        3                          // multiply by 8 for index past __longjmpv20
        mflr r6

            addi r6,
        r6, __longjmpv20 - __setLR - (20 * 8) add r6, r6,
        r5 mtlr r6 blr             // jump to index past __longjmpv20

            __longjmpv20 :
    #endif
      la                  r31,
      context->VR[20] lvx vr20,
      0,
      r31 la              r31,
      context->VR[21] lvx vr21,
      0,
      r31 la              r31,
      context->VR[22] lvx vr22,
      0,
      r31 la              r31,
      context->VR[23] lvx vr23,
      0,
      r31 la              r31,
      context->VR[24] lvx vr24,
      0,
      r31 la              r31,
      context->VR[25] lvx vr25,
      0,
      r31 la              r31,
      context->VR[26] lvx vr26,
      0,
      r31 la              r31,
      context->VR[27] lvx vr27,
      0,
      r31 la              r31,
      context->VR[28] lvx vr28,
      0,
      r31 la              r31,
      context->VR[29] lvx vr29,
      0,
      r31 la              r31,
      context->VR[30] lvx vr30,
      0,
      r31 la              r31,
      context->VR[31] lvx vr31,
      0,
      r31
#endif
    ...
    #if __PPCe500__
          la                  r31,
      context->GPR[13] evlddx r13,
      0,
      r31 la                  r31,
      context->GPR[14] evlddx r14,
      0,
      r31 la                  r31,
      context->GPR[15] evlddx r15,
      0,
      r31 la                  r31,
      context->GPR[16] evlddx r16,
      0,
      r31 la                  r31,
      context->GPR[17] evlddx r17,
      0,
      r31 la                  r31,
      context->GPR[18] evlddx r18,
      0,
      r31 la                  r31,
      context->GPR[19] evlddx r19,
      0,
      r31 la                  r31,
      context->GPR[20] evlddx r20,
      0,
      r31 la                  r31,
      context->GPR[21] evlddx r21,
      0,
      r31 la                  r31,
      context->GPR[22] evlddx r22,
      0,
      r31 la                  r31,
      context->GPR[23] evlddx r23,
      0,
      r31 la                  r31,
      context->GPR[24] evlddx r24,
      0,
      r31 la                  r31,
      context->GPR[25] evlddx r25,
      0,
      r31 la                  r31,
      context->GPR[26] evlddx r26,
      0,
      r31 la                  r31,
      context->GPR[27] evlddx r27,
      0,
      r31 la                  r31,
      context->GPR[28] evlddx r28,
      0,
      r31 la                  r31,
      context->GPR[29] evlddx r29,
      0,
      r31 la                  r31,
      context->GPR[30] evlddx r30,
      0,
      r31 la                  r31,
      context->GPR[31] evlddx r31,
      0,
      r31
    #else  /* __PPCe500__ */
        #if !__option(use_lmw_stmw) || __option(little_endian)
            lwz              r13,
        context->GPR[13] lwz r14, context->GPR[14] lwz r15, context->GPR[15] lwz r16,
        context->GPR[16] lwz r17, context->GPR[17] lwz r18, context->GPR[18] lwz r19,
        context->GPR[19] lwz r20, context->GPR[20] lwz r21, context->GPR[21] lwz r22,
        context->GPR[22] lwz r23, context->GPR[23] lwz r24, context->GPR[24] lwz r25,
        context->GPR[25] lwz r26, context->GPR[26] lwz r27, context->GPR[27] lwz r28,
        context->GPR[28] lwz r29, context->GPR[29] lwz r30, context->GPR[30] lwz r31,
        context->GPR[31]
        #else
            lmw r13,
        context->GPR[13]
    ...
    #else
        lfd                  fp14,
        context->FPR[14] lfd fp15, context->FPR[15] lfd fp16, context->FPR[16] lfd fp17,
        context->FPR[17] lfd fp18, context->FPR[18] lfd fp19, context->FPR[19] lfd fp20,
        context->FPR[20] lfd fp21, context->FPR[21] lfd fp22, context->FPR[22] lfd fp23,
        context->FPR[23] lfd fp24, context->FPR[24] lfd fp25, context->FPR[25] lfd fp26,
        context->FPR[26] lfd fp27, context->FPR[27] lfd fp28, context->FPR[28] lfd fp29,
        context->FPR[29] lfd fp30, context->FPR[30] lfd fp31,
        context->FPR[31]
    #endif            /* __PPCGEKKO__ */
          ...
          mtlr r8     //	restore PC (LR)
              ...
              lwz          r5,
      context->vrsave oris r5,
      r5,
      0x8000          // indicate vr0 also in-use
      mtvrsave r5
    #endif
          la              r5,
      context->vscr lvewx vr0,
      0,
      r5              // load 32 bits into lo 32 bits of v0
          mtvscr vr0

              lwz r5,
      context->vrsave // restore vrsave, remove usage of bit 0
          mtvrsave r5
#endif
...
ExPPC_ThrowHandler (ThrowContext* context)
{
    ...
    {                            //	rethrow, find most recent exception
...
#if __PPCe500__
    #if (__option(little_endian))
        iter.current_R31 = context->GPR[31].l[0];
    #else
        iter.current_R31 = context->GPR[31].l[1];
    #endif
#else
        ...
        catchinfo          = (CatchInfo*)(iter.current_FP +
        ...
        switch (action)
        {
            ...
            case EXACTION_SPECIFICATION:
                ...
                { //	unexpected specification
                    ExPPC_HandleUnexpected (context,
                                            &info,
                                            (ex_specification*)iter.info.action_pointer);
    ...
    if (action == EXACTION_CATCHBLOCK_32)
    {
        ...
        //	jump to exception handler
        ExPPC_LongJump (context,
                        info.TOC,
                        info.current_function + catchblock_32->catch_pcoffset);
    }
    else
    {
...
__throw (char* throwtype, void* location, void* dtor)
{
    ...
    #if __PPC_EABI__
        #if !__option(use_lmw_stmw) || __option(little_endian)
    stw r13, throwcontext.GPR[13] stw r14, throwcontext.GPR[14] stw r15,
        throwcontext.GPR[15] stw r16, throwcontext.GPR[16] stw r17,
        throwcontext.GPR[17] stw r18, throwcontext.GPR[18] stw r19,
        throwcontext.GPR[19] stw r20, throwcontext.GPR[20] stw r21,
        throwcontext.GPR[21] stw r22, throwcontext.GPR[22] stw r23,
        throwcontext.GPR[23] stw r24, throwcontext.GPR[24] stw r25,
        throwcontext.GPR[25] stw r26, throwcontext.GPR[26] stw r27,
        throwcontext.GPR[27] stw r28, throwcontext.GPR[28] stw r29,
        throwcontext.GPR[29] stw r30, throwcontext.GPR[30] stw r31,
        throwcontext.GPR[31]
        #else
    stmw r13,
        throwcontext.GPR[13]
    ...
    stmw r13,
        ...
        stfd                      fp14,
        throwcontext.FPR[14] stfd fp15, throwcontext.FPR[15] stfd fp16,
        throwcontext.FPR[16] stfd fp17, throwcontext.FPR[17] stfd fp18,
        throwcontext.FPR[18] stfd fp19, throwcontext.FPR[19] stfd fp20,
        throwcontext.FPR[20] stfd fp21, throwcontext.FPR[21] stfd fp22,
        throwcontext.FPR[22] stfd fp23, throwcontext.FPR[23] stfd fp24,
        throwcontext.FPR[24] stfd fp25, throwcontext.FPR[25] stfd fp26,
        throwcontext.FPR[26] stfd fp27, throwcontext.FPR[27] stfd fp28,
        throwcontext.FPR[28] stfd fp29, throwcontext.FPR[29] stfd fp30,
        throwcontext.FPR[30] stfd fp31,
        throwcontext.FPR[31]
    #endif
        ...
        throwcontext.CR
            ...
            lwz  r3,
        0(sp)lwz r4, RETURN_ADDRESS (r3) stw r3, throwcontext.SP stw r3,
        throwcontext.throwSP stw r4,
        throwcontext.returnaddr
            //	throwcontext.throwtype = throwtype;
            lwz       r3,
        throwtype stw r3,
        throwcontext.throwtype
            //	throwcontext.location = location;
            lwz      r3,
        location stw r3,
        throwcontext.location
      ...
      stw                          r5,
        throwcontext.vrsave clrlwi r5, r5,
        20                 // clear left 20 bits
        cntlzw r5,
        r5                 // find first nonvolatile VR used
            slwi r5,
        r5,
        3                  // multiply by 8 for index past __setjmpv20
        mflr r4

            addi r4,
        r4, __setjmpv20 - __setLR - (20 * 8) add r4, r4,
        r5 mtlr r4
    ...
    #endif
      la                       r3,
      throwcontext.VR[20] stvx vr20,
      r0,
      r3 la                    r3,
      throwcontext.VR[21] stvx vr21,
      r0,
      r3 la                    r3,
      throwcontext.VR[22] stvx vr22,
      r0,
      r3 la                    r3,
      throwcontext.VR[23] stvx vr23,
      r0,
      r3 la                    r3,
      throwcontext.VR[24] stvx vr24,
      r0,
      r3 la                    r3,
      throwcontext.VR[25] stvx vr25,
      r0,
      r3 la                    r3,
      throwcontext.VR[26] stvx vr26,
      r0,
      r3 la                    r3,
      throwcontext.VR[27] stvx vr27,
      r0,
      r3 la                    r3,
      throwcontext.VR[28] stvx vr28,
      r0,
      r3 la                    r3,
      throwcontext.VR[29] stvx vr29,
      r0,
      r3 la                    r3,
      throwcontext.VR[30] stvx vr30,
      r0,
      r3 la                    r3,
      throwcontext.VR[31] stvx vr31,
      r0,
      r3
      ...
      mtvrsave r6
          ...
          mfvscr vr0       // mfvscr is context-synchronizing, so this will take many cycles
              la               r5,
      throwcontext.vscr stvewx vr0,
      r0,
      r5                   // store 32 bits out of lo 32 bits of v0

          li r5,
      0x0000               // indicate that no vector registers are being used now
      mtvrsave r5 // This is OK because we just saved the non-volatile set and exercised our
                  // right to trash all the volatile vector registers

                      //	call __ex_throwhandler(&throwcontext);
                      la r3,
      throwcontext bl ExPPC_ThrowHandler nop
          //	(will never get here)
          frfree blr
}
...
__throw (char* throwtype, void* location, void* dtor)
{
    ...
    //	save GPRs so we can restore them during stack unwind
        ...
        #if __PPCe500__
        la                           r3,
        throwcontext.GPR[13] evstddx r13, 0, r3 la r3, throwcontext.GPR[14] evstddx r14, 0,
        r3 la r3, throwcontext.GPR[15] evstddx r15, 0, r3 la r3,
        throwcontext.GPR[16] evstddx r16, 0, r3 la r3, throwcontext.GPR[17] evstddx r17, 0,
        r3 la r3, throwcontext.GPR[18] evstddx r18, 0, r3 la r3,
        throwcontext.GPR[19] evstddx r19, 0, r3 la r3, throwcontext.GPR[20] evstddx r20, 0,
        r3 la r3, throwcontext.GPR[21] evstddx r21, 0, r3 la r3,
        throwcontext.GPR[22] evstddx r22, 0, r3 la r3, throwcontext.GPR[23] evstddx r23, 0,
        r3 la r3, throwcontext.GPR[24] evstddx r24, 0, r3 la r3,
        throwcontext.GPR[25] evstddx r25, 0, r3 la r3, throwcontext.GPR[26] evstddx r26, 0,
        r3 la r3, throwcontext.GPR[27] evstddx r27, 0, r3 la r3,
        throwcontext.GPR[28] evstddx r28, 0, r3 la r3, throwcontext.GPR[29] evstddx r29, 0,
        r3 la r3, throwcontext.GPR[30] evstddx r30, 0, r3 la r3,
        throwcontext.GPR[31] evstddx r3, 0,
        r3
        #else  /* __PPCe500__ */
            #if !__option(use_lmw_stmw) || __option(little_endian)
        stw                      r13,
        throwcontext.GPR[13] stw r14, throwcontext.GPR[14] stw r15,
        throwcontext.GPR[15] stw r16, throwcontext.GPR[16] stw r17,
        throwcontext.GPR[17] stw r18, throwcontext.GPR[18] stw r19,
        throwcontext.GPR[19] stw r20, throwcontext.GPR[20] stw r21,
        throwcontext.GPR[21] stw r22, throwcontext.GPR[22] stw r23,
        ...
        throwcontext.FPR[14].d la r3, throwcontext.FPR[14].v psq_stx fp14, 0, r3, 0,
        0

        stfd                      fp15,
        throwcontext.FPR[15].d la r3, throwcontext.FPR[15].v psq_stx fp15, 0, r3, 0,
        0

        stfd                      fp16,
        throwcontext.FPR[16].d la r3, throwcontext.FPR[16].v psq_stx fp16, 0, r3, 0,
        0

        stfd                      fp17,
        throwcontext.FPR[17].d la r3, throwcontext.FPR[17].v psq_stx fp17, 0, r3, 0,
        0

        stfd                      fp18,
        throwcontext.FPR[18].d la r3, throwcontext.FPR[18].v psq_stx fp18, 0, r3, 0,
        0

        stfd                      fp19,
        throwcontext.FPR[19].d la r3, throwcontext.FPR[19].v psq_stx fp19, 0, r3, 0,
        0

        stfd                      fp20,
        throwcontext.FPR[20].d la r3, throwcontext.FPR[20].v psq_stx fp20, 0, r3, 0,
        0

        stfd                      fp21,
        throwcontext.FPR[21].d la r3, throwcontext.FPR[21].v psq_stx fp21, 0, r3, 0,
        0

        stfd                      fp22,
        throwcontext.FPR[22].d la r3, throwcontext.FPR[22].v psq_stx fp22, 0, r3, 0,
        0

        stfd                      fp23,
        throwcontext.FPR[23].d la r3, throwcontext.FPR[23].v psq_stx fp23, 0, r3, 0,
        0

        stfd                      fp24,
        throwcontext.FPR[24].d la r3, throwcontext.FPR[24].v psq_stx fp24, 0, r3, 0,
        0

        stfd                      fp25,
        throwcontext.FPR[25].d la r3, throwcontext.FPR[25].v psq_stx fp25, 0, r3, 0,
        0

        stfd                      fp26,
        throwcontext.FPR[26].d la r3, throwcontext.FPR[26].v psq_stx fp26, 0, r3, 0,
        0

        stfd                      fp27,
        throwcontext.FPR[27].d la r3, throwcontext.FPR[27].v psq_stx fp27, 0, r3, 0,
        0

        stfd                      fp28,
        throwcontext.FPR[28].d la r3, throwcontext.FPR[28].v psq_stx fp28, 0, r3, 0,
        0

        stfd                      fp29,
        throwcontext.FPR[29].d la r3, throwcontext.FPR[29].v psq_stx fp29, 0, r3, 0,
        0

        stfd                      fp30,
        throwcontext.FPR[30].d la r3, throwcontext.FPR[30].v psq_stx fp30, 0, r3, 0,
        0

        stfd                      fp31,
        throwcontext.FPR[31].d la r3, throwcontext.FPR[31].v psq_stx fp31, 0, r3, 0,
        0
        #else
        stfd                      fp14,
        throwcontext.FPR[14] stfd fp15, throwcontext.FPR[15] stfd fp16,
    ...
    //	throwcontext.SP = throwcontext.throwSP = <stack pointer of caller>;
    //	throwcontext.returnaddr = <return address into caller>;
    lwz r3, 0(sp)lwz r4, RETURN_ADDRESS (r3) stw r3, throwcontext.SP;
    stw r3, throwcontext.throwSP;
    stw r4, throwcontext.returnaddr;
    //	throwcontext.throwtype = throwtype;
    lwz r3, throwtype stw r3,
        throwcontext.throwtype
            //	throwcontext.location = location;
            lwz      r3,
        location stw r3,
