...
timed_mutex::lock ()
{
    ...
    while (locked_) pthread_cond_wait (&c_, &m_);
    locked_ = true;
...
recursive_mutex::lock ()
{
    ...
    {
        while (count_ > 0) pthread_cond_wait (&c_, &m_);
        count_ = 1;
        owner_ = self;
    }
    pthread_mutex_unlock (&m_);
...
loop1:
    ...
    #else  // __POWERPC__
    private_lock lk (mut_);
    while (activity_ & (write_entered_ | upgradable_entered_)) entry_.wait (lk);
    activity_ |= write_entered_;
    while (activity_ & max_readers_) first_wait_.wait (lk);
    #endif // __POWERPC__
}
...
loop:
    asm {lwarx    a, 0, p}
    if (a & (write_entered_ | max_readers_)) return false;
    a |= write_entered_;
    asm
    {
		stwcx.   a, 0, p
...
loop:
    asm {lwarx    a, 0, p}
    if ((a & max_readers_) != 1) return false;
    a &= ‾(upgradable_entered_ | max_readers_);
    a |= write_entered_;
    asm
    {
...
loop:
    asm {lwarx    a, 0, p}
    if (a & (write_entered_ | upgradable_entered_)) return false;
    a |= upgradable_entered_;
    asm
    {
		stwcx.   a, 0, p
...
loop:
    asm {lwarx    a, 0, p}
    if ((a & (write_entered_ | upgradable_entered_)) || (a & max_readers_) != 1) return false;
    a &= ‾max_readers_;
    a |= write_entered_;
    asm
    {
