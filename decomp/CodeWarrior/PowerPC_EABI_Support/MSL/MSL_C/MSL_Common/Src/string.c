...
char*
strncpy (char* _MSL_RESTRICT dst, const char* _MSL_RESTRICT src, size_t n)
{
        #if !defined(__POWERPC__)
...
char*
strcat (char* _MSL_RESTRICT dst, const char* _MSL_RESTRICT src)
{
            #if !defined(__POWERPC__)
...
char*
strncat (char* _MSL_RESTRICT dst, const char* _MSL_RESTRICT src, size_t n)
{
        #if !defined(__POWERPC__)
...
{
            #if (!defined(__POWERPC__) && !defined(__MIPS__) && !defined(__MC68K__) &&         \
                 !defined(__arm))
    ...
    return (c1 - c2);
            ...
            #elif (defined(__POWERPC__) || defined(__MIPS__) || defined(__MC68K__) ||          \
                   defined(__arm)) /*- cc 010821 -*/
...
size_t _MSL_CDECL
strxfrm (char* _MSL_RESTRICT str1, const char* _MSL_RESTRICT str2, size_t n)
{
    int len = strlen (str2);
    ...
    weight1                    = malloc (len);
    weight2                    = malloc (len);
    for (wt1ptr = weight1; wt1ptr < (weight1 + len); wt1ptr++) *wt1ptr = '\0';
    for (wt2ptr = weight2; wt2ptr < (weight2 + len); wt2ptr++) *wt2ptr = '\0';
    wt1ptr = weight1;
    wt2ptr = weight2;
    ...
    if (current_locale_ptr->coll_cmpt_ptr->char_spec_accents)
        for (tempptr = wt1ptr - 1; tempptr >= weight1; tempptr--) *collptr++ = *tempptr;
    else
        for (tempptr = weight1; tempptr < wt1ptr; tempptr++) *collptr++ = *tempptr;

    for (tempptr = weight2; tempptr < wt2ptr; tempptr++) *collptr++ = *tempptr;
...
char* _MSL_CDECL
strtok (char* _MSL_RESTRICT str, const char* _MSL_RESTRICT set)
{
...
char* _MSL_CDECL
strtok_s (char* _MSL_RESTRICT str, const char* _MSL_RESTRICT set, char** _MSL_RESTRICT ptr)
{
    unsigned char *p, *q;
    int            c;
...
errno_t
strncpy_s (char* _MSL_RESTRICT dst, rsize_t dst_max, const char* _MSL_RESTRICT src, rsize_t n)
{
    rsize_t i;
...
errno_t
strncat_s (char* _MSL_RESTRICT dst, rsize_t dst_max, const char* _MSL_RESTRICT src, rsize_t n)
{
    rsize_t i;
    rsize_t dst_cat_max;
...
errno_t _MSL_CDECL
strerror_s (char* s, rsize_t maxsize, errno_t errnum)
{
    int  realsize;
 ...
 * JWW 040317 Updated the __STDC_WANT_LIB_EXT1__ secure library based on new secure
 *specifications mm  040602 Freed a piece of malloced storage JWW 040908 Obtain fast Mach-O
 *versions of strcpy, strcmp, etc. from the System.framework mm  050126 Correct alignment cases.
