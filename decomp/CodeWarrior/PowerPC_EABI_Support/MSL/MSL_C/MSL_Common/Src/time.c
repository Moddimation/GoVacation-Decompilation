...
char*
__MSL_extract_name (char* source, int position) /*- mm 921219 -*/
{
    int         index;
...
void
__msl_time2tm (time_t inTime, struct tm* tm) /*- mm 000127 -*/
{
...
void
__msl_asciitime (struct tm tm, char* str)
{
    ...
    strcpy (day_name,
            __MSL_extract_name (current_locale_ptr->time_cmpt_ptr->Day_Names, tm.tm_wday * 2));
        /*- mm 011130 -*/                                   /*- mm 021219 -*/
    strcpy (month_name,
            __MSL_extract_name (current_locale_ptr->time_cmpt_ptr->MonthNames, tm.tm_mon * 2));
        /*- mm 011130 -*/                                   /*- mm 021219 -*/
                                                            /*- mm 011130 -*/
    sprintf (str,
             "%s %s%3d %.2d:%.2d:%.2d %d\n",                /*- mm 011130 -*/
...
errno_t _MSL_CDECL
asctime_s (char* s, rsize_t maxsize, const struct tm* tm)
{
    int              n;
    int              year;
    ...
    {
        strcpy (day_name,
                __MSL_extract_name (current_locale_ptr->time_cmpt_ptr->Day_Names,
                                    asc_tm.tm_wday * 2));
        strcpy (month_name,
                __MSL_extract_name (current_locale_ptr->time_cmpt_ptr->MonthNames,
                                    asc_tm.tm_mon * 2));
...
errno_t _MSL_CDECL
ctime_s (char* s, rsize_t maxsize, const time_t* timer)
{
    if (timer == NULL)
    {
        __msl_runtime_constraint_violation_s (NULL, NULL, ERANGE);
...
struct tm* _MSL_CDECL
gmtime_s (const time_t* _MSL_RESTRICT timer, struct tm* _MSL_RESTRICT tm)
{
    time_t time;
...
struct tm* _MSL_CDECL
localtime_s (const time_t* _MSL_RESTRICT timer, struct tm* _MSL_RESTRICT tm)
{
    time_t time;
...
int
__msl_week_num (const struct tm* tm, int starting_day)
{
    int days  = tm->tm_yday;
...
time_t
__MSL_ISO8601NewYear (int year)                                       /*- mm 021220 -*/
{
    struct tm ts0;
    time_t    timeval;
...
int
__MSL_ISO8601Week (const struct tm* tmptr, int* WYear)                /*- mm 021220 -*/
{
    struct WeekYear
    {
        int    Year;
...
size_t _MSL_CDECL
strftime (char* _MSL_RESTRICT            str,
          size_t                         max_size,
          ...
          const struct tm* _MSL_RESTRICT timeptr)
{
        ...
        switch (*++format_ptr)
        {
            ...
            case 'c':
                num_chars        = strftime (str,
                                      space_remaining + 1,
                                      current_locale_ptr->time_cmpt_ptr->DateTime_Format,
                                      &tm);                    /*- mm 020606 -*/
                space_remaining -= num_chars;                         /*- mm 980501 -*/
                break;
            ...
            case 'd':
                num_chars =
                    emit (str, 2, &space_remaining, "%.2d", tm.tm_mday);   /*- mm 980501 -*/
                break;

            case 'D':
                num_chars =
                    strftime (str, space_remaining + 1, "%m/%d/%y", &tm);  /*- mm 000830 -*/
                break;

            case 'e':                                                      /*- mm 000404 -*/
                num_chars =
                    emit (str, 2, &space_remaining, "%2d", tm.tm_mday);    /*- mm 000404 -*/
                break;                                                     /*- mm 000404 -*/
            ...
            case 'H':
                num_chars =
                    emit (str, 2, &space_remaining, "%.2d", tm.tm_hour);     /*- mm 980501 -*/
                break;
            ...
            case 'M':
                num_chars =
                    emit (str, 2, &space_remaining, "%.2d", tm.tm_min);      /*- mm 980501 -*/
                break;
            ...
            case 'p':
                name = __MSL_extract_name (current_locale_ptr->time_cmpt_ptr->am_pm,
                                           tm.tm_hour < 12 ? 0 : 1);
                    /*- mm 011130 -*/                                        /*- mm 021219 -*/
                num_chars =
                    emit (str, strlen (name), &space_remaining, "%s", name); /*- mm 011130 -*/
                break;

            case 'r':
                num_chars        = strftime (str,
                                      space_remaining + 1,
                                      current_locale_ptr->time_cmpt_ptr->Twelve_hr_format,
                                      &tm);                           /*- mm 020606 -*/
            ...
            case 'S':
                num_chars =
                    emit (str, 2, &space_remaining, "%.2d", tm.tm_sec);      /*- mm 980501 -*/
                break;
            ...
            case 'T':
                num_chars =
                    strftime (str, space_remaining + 1, "%H:%M:%S", &tm);    /*- mm 011130 -*/
            ...
            case 'w':
                num_chars =
                    emit (str, 1, &space_remaining, "%.1d", tm.tm_wday);     /*- mm 980501 -*/
                break;
            ...
            case 'W':
                num_chars = emit (str,
                                  ...
                                  &space_remaining,
                                  "%.2d",
                                  __msl_week_num (&tm, 1));                  /*- mm 980501 -*/
                break;

            case 'x':
                num_chars        = strftime (str,
                                      space_remaining + 1,
                                      current_locale_ptr->time_cmpt_ptr->Date_Format,
                                      &tm);                           /*- mm 020606 -*/
                space_remaining -= num_chars;
                break;
            ...
            case 'X':
                num_chars        = strftime (str,
                                      space_remaining + 1,
                                      current_locale_ptr->time_cmpt_ptr->Time_Format,
                                      &tm);                           /*- mm 020606 -*/
                space_remaining -= num_chars;                                /*- mm 980501 -*/
                break;
            ...
            case 'Z':
                if (*current_locale_ptr->time_cmpt_ptr->TimeZone == '\0')   /*- mm 011130 -*/
                {
                    ...
                    num_chars =
                        emit (str,
                              strlen (current_locale_ptr->time_cmpt_ptr->TimeZone),
                              &space_remaining,
                              "%s",                                         /*- mm 011130 -*/
            ...
            case '%':                                                       /*- mf 092497 -*/
                num_chars =
                    emit (str, 2, &space_remaining, "%%", *format_ptr);     /*- mm 980501 -*/
                break;

            default:
                num_chars =
                    emit (str, 2, &space_remaining, "%%%c", *format_ptr);   /*- mm 980501 -*/
                break;
        }
    ...
    *str = 0;
    if (max_size < chars_written)
        ...
        * mm  980501 Corrected strftime to return correct number of characters written and make test
        *for overrun        MW06496        mf  980512 wince changes        mm  990705 Implemented %T, %F, %u        mm  990709
        *Implemented %g, %G, %V        vss 990811 Implement %r %R        vss 990811 Added C9X guards around old ANSI
        *C and new C9X functionality        mm  990910 Added _No_Time_OS_Support wrappers to avoid call to
        *gmtime        mm  000127 Changed Mac epoch from 1900Jan01 to 1970Jan01 to accord with POSIX and MSL
        *on Windows  MW07637        mm  000404 Changes to strftime to accord with C9x        as  000823 Corrected
        *guard in strftime(), No_Time_OS_Support is now _No_Time_OS_Support        mm  000830 Changes to
        *strftime to accord with C9x        cc  000326 removed dest_os to be_os        mm  010420 Corrected setting
        *of tm_isdst        mm  010425 Correction to mm 010420---tm_isdst should always be zero for UTC        cc
        *010510 Changed __add to __msl_add        cc  010510 Changed __div to __msl_div        cc  010510 Changed
        *__ladd to __msl_ladd        cc  010510 Changed __lmul to __msl_lmul        cc  010510 Changed __mod to
        *__msl_mod        mm  010516 Thread local storage for localtime function        cc  010531 Added
        *_GetThreadLocalData's flag        cc  010822 Added _No_Floating_Point and _No_Time_OS_Support
        *wrappers        JWW 010918 Use time_api.h to get clock and time APIs        cc  011128 Made __tls _MSL_TLS
        ...
        * mm  021220 Changed names of ISO8601NewYear() and ISO8601Week() to __MSL_ISO8601NewYear() and
        *__MSL_ISO8601Week()        cc  030122 Removed _MSL_C9X_ rapper around ISO8601NewYear        ejs 030113 Hide
        *ISO8601Week() when !defined(_No_Floating_Point)        mm  030218 Changed C9X in wrapper to C99        JWW
        *031030 Added the __STDC_WANT_LIB_EXT1__ secure library extensions        cc  040204 The
        *_No_Time_OS_Support flag has been deprecated and replaced with _MSL_OS_TIME_SUPPORT        JWW 040317
        *Updated the __STDC_WANT_LIB_EXT1__ secure library based on new secure specifications
        */
