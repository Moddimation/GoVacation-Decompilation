...
public:
    ...
    wait ()
    {
        ...
        while (!started_) condition_.wait (lock);
    }
...
public:
    handler&
    operator[] (const thread_exit_key_type& x)
    {
        return map_[x];
    }
...
inline thread_exit_key_helper::thread_exit_key_helper ()
{
...
thread_group::thread_group ()
{
    ...
    compile_assert<sizeof (impl) == sizeof (C)> e1;
    (void)e1;
}
...
thread_group::‾thread_group()
{
    typedef _STD::vector<thread*> C;
    C&                            v = (C&)impl_;
    for (C::iterator i = v.begin(), e = v.end(); i != e; ++i) delete *i;
    v.‾C();
}
...
thread_group::join_all ()
{
    ...
    mutex::scoped_lock            lock (mutex_);
    C&                            v = (C&)impl_;
    for (C::iterator i = v.begin(), e = v.end(); i != e; ++i) (*i)->join();
}
...
call_once (void (*func)(), once_flag& flag)
{
    if (flag.not_done_)
    {
        ...
        while (flag.not_done_) thread::yield();
    }
}
