...
errno_t _MSL_CDECL
tmpnam_s (char* name, rsize_t maxsize)
{
...
FILE* _MSL_CDECL
fopen (const char* _MSL_RESTRICT name, const char* _MSL_RESTRICT mode)
{
    FILE* file;
...
errno_t _MSL_CDECL
fopen_s (FILE* _MSL_RESTRICT* _MSL_RESTRICT file,
         const char* _MSL_RESTRICT          name,
         const char* _MSL_RESTRICT          mode)
{
    FILE* newfile;
...
FILE* _MSL_CDECL
freopen (const char* _MSL_RESTRICT name,
         const char* _MSL_RESTRICT mode,
         FILE* _MSL_RESTRICT       file)
{
    __file_modes modes;
...
errno_t _MSL_CDECL
freopen_s (FILE* _MSL_RESTRICT* _MSL_RESTRICT newfile,
           const char* _MSL_RESTRICT          name,
           const char* _MSL_RESTRICT          mode,
           FILE* _MSL_RESTRICT                file)
{
...
void _MSL_CDECL
__set_ref_con (FILE* file, __ref_con ref_con)
{
...
__msl_strdup (const char* str) /*- cc 010725 -*/
{
    ...
    return rval;
                                ...
                                * mm  980430 The Standard does not define any action for fflush on a file opened in read only
                                *mode so do                                nothing.  MW02733                                mf  980512 wince changes                                mm  980707 Corrected initialization of
                                *__temp_file_mode to allow for wide characters                                mm  981029 Excluded fflush() for __m56800__                                mm
                                *990119 When a file is opened in append mode, make sure that it is positioned to eof
                                *IL9901-1347                                bds 990121 Added BEOS wrapper                                mf  990301 removed calls to _GetThreadLocalData for
                                *single threaded lib ansix86st.lib                                mm  990609 Added code for __reopen, which changes  stdin,
                                *stdout, or stderr into binary mode.                                blc 991210 Fixed TLS leak into Win32 libraries through
                                *obsolete buffer declaration in tmpnam()                                cc  010326 removed dest_os to be_os                                cc  010405 readded
                                *rename                                mm  010518 Add _MWMT wrapper to #include <ThreadLocalData.h>                                cc  010531 Added
                                *_GetThreadLocalData's flag                                cc  010605 Made __msl_itoa = to itoa, __msl_strnicmp = to strnicmp,
                                *__msl_strrev = to strrev for                                MSL C & added for #include <ctype.h> and <string.h>                                cc  010725
                                *Added __msl_strdup                                JWW 011115 Use the BSD rename() function when compiling for Mach-O and not
                                *using Carbon APIs                                cc  011203 Added _MSL_CDECL for new name mangling                                JWW 020130 Changed _MWMT
                                *to _MSL_THREADSAFE for consistency's sake                                JWW 020604 Added _MSL_LOCALDATA macro to make
                                *multithread code easier to read                                JWW 020906 Use generic reference constant instead of specific
                                *idle_proc in file I/O                                JWW 020917 Added _MSL_RESTRICT to get the restrict type specifier for
                                *certain C99 functions                                JWW 021031 Made thread local data available for Mach-O                                JWW 030224
                                *Changed __NO_WIDE_CHAR flag into the new more configurable _MSL_WIDE_CHAR                                JWW 031030 Added the
                                ...
                                *based on new secure specifications                                mm  040428 Avoided nesting by entering __init_file in a
                                *critical region since __init_file calls setvbuf.                                mm  040429 More avoidance of nested critical
                                *regions
                                */
