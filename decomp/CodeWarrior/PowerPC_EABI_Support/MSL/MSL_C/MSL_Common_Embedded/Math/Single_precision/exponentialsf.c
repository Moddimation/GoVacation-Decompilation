...
/*
  ...
  Purpose: portable single precision versions of ANSI standard powf(x,y),expf(x) functions
           and support functions.  log2f(x) is a supported part of C9X and is therefore supplied
  as a standard routine in a separate module(log2f.c).
  ...
  error analysis:
    ...
    Most of the rounding errors are concentrated in the regions where expf^x approaches the
  upper limit of representable numbers(i.e. expf^^x ‾= FLT_MAX).  In this case the variable
  transform from x to z=log2e*x is the principle cause.


  __2_to_x(float x) is basically the same routine but eliminates the expensive variable
  transform step thereby leading to better performance and accuracy.  This is used by the pow
  function. i.e. pow(x,y)=x^^y=2^^(y*log2(x)).
...
static const float log2e[]      = { *(float*)&log2e_bits[0],
                                    *(float*)&log2e_bits[1],
                                    *(float*)&log2e_bits[2],
                                    *(float*)&log2e_bits[3] };
        #endif
...
static const float __two_to_x[] = {
    .693147177596419,   .240226604484390,   .0555029156907419,
    .00962502261002979, .00131310538528088, .000183008060120550
};
...
__2_to_x (float x)
{
    ...
    _INT32 index = 0; // polynomial is only valid for x > 0 -> if frac_part < 0 we multiply by
                      // 2^-.5 and add .5 to frac_part
    float  z;         // intermediate value evaluated by polynomial
    float  frac_part;
    int_part  = (signbit (x))
                    ? (_INT32)(x - .5f)
                    : (_INT32)(x + .5f); // won't round to even in half way cases(doesn't matter)
    ...
    z = frac_part +
        scale_frac[index]; // puts z in [0,.5] , incurs 1 ulp rounding error in last bit
    z = z *
        (__two_to_x[0] +
         z * (__two_to_x[1] +
              z * (__two_to_x[2] +
                   z * (__two_to_x[3] +
                        z * (__two_to_x[4] + z * __two_to_x[5]))))); // calculate 2^^x -1 -> z
                                                                     // in [0.0 ,.414213562]

                                                                     // reassemble
        #if __MIPS__
    return __int2float__ (int_part) *
           (.75f * two_to_frac[index] + two_to_frac[index] * (z + .25f));
        #else
...
expf (float x)
{
    ...
    z = x * log2e[0] +
        (x * log2e[1] +
         (x * log2e[2] + x * log2e[3])); // extra bits for log2(e) forces correct rounding

    int_part = (signbit (z))
                   ? (_INT32)(z - .5f)
                   : (_INT32)(z + .5f); // won't round to even in half way cases(doesn't matter)

    // the order and number of terms below has an affect on accuracy.  This should not be
    // changed.

    frac_part =
        (int_x * log2e[0] - (float)int_part) +
        (int_x * log2e[1] +
         (frac_x * log2e[0] +
