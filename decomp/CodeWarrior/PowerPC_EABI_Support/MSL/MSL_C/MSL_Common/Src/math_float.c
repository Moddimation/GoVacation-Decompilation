...
cbrtf (float x) _MSL_CANT_THROW
{
    ...
    *ix      |= 0x3F000000;
    ...
    xd         = x;
    x          = (float)((0.164705865585441 +
                 (13.2513868634597 +
                  (115.1358553761178 + (181.9414139157457 + 41.95971194004274 * xd) * xd) *
                      xd) *
                     xd) /
                (1.0 +
                 (33.30169492280659 +
                  (161.6940396106312 + (142.8167287366127 + 13.64061797885738 * xd) * xd) *
                      xd) *
                     xd));
    exponent  /= 3;
    exponent <<= 23;
...
_MSL_IMP_EXP_C float _MSL_MATH_CDECL
erff (float x) _MSL_CANT_THROW
{
    return ((float)erf (x));         /* cc 060303 */
}

_MSL_IMP_EXP_C float _MSL_MATH_CDECL
erfcf (float x) _MSL_CANT_THROW
{
    return ((float)(1.0 - erf (x))); /* cc 060303 */
}

_MSL_IMP_EXP_C float _MSL_MATH_CDECL
exp2f (float x) _MSL_CANT_THROW
{
...
_MSL_IMP_EXP_C float _MSL_MATH_CDECL
lgammaf (float x) _MSL_CANT_THROW
{
    return ((float)(logl (tgammal (x))));
}

_MSL_IMP_EXP_C float _MSL_MATH_CDECL
nanf (const char* str) _MSL_CANT_THROW
{
    char temparg[32] = "NAN(";
    strcat (temparg, str);
    strcat (temparg, ")");
...
_MSL_IMP_EXP_C float _MSL_MATH_CDECL
nextafterf (float x, float y) _MSL_CANT_THROW
{
    return nexttowardf (x, (long double)y);
}

_MSL_IMP_EXP_C float _MSL_MATH_CDECL
nexttowardf (float x, long double y) _MSL_CANT_THROW
{
    int            increase;
    int            positive;
    ...
    if ((ly[1] & 0x7FF00000) == 0x7FF00000 && (ly[1] & 0x000FFFFF) | ly[0] || x == y)
        return (float)y;
            #else
    if ((ly[0] & 0x7FF00000) == 0x7FF00000 && (ly[0] & 0x000FFFFF) | ly[1] || x == y)
        return (float)y;
...
_MSL_IMP_EXP_C float _MSL_MATH_CDECL
tgammaf (float x) _MSL_CANT_THROW
{
    return ((float)tgammal (x));
                                      ...
                                      * hh  040410 Added several more intrinsics, changed implementation of
                                      *__msl_generic_count_leading_zero32                                      and __msl_generic_count_trailing_zero32                                      cc  040504 Added
                                      *_MSL_FLOATING_POINT flag check                                      cc  040518 File has been split                                      cc  060303 Using suns erf                                      cc
                                      *070321 Added rintf
                                      */
