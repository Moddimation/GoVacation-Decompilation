...
*/
    ...
    */
        ...
        * JWW 010926 Changed _POSIX to _MSL_POSIX to control what happens with POSIX functions in ANSI
        *headers        cc  011022 Added _MSL_NO_FLOAT_HEX, _MSL_USES_EXIT_PROCESS, _MSL_NEEDS_EXTRAS        JWW
        *011027 New _MSL_USING_MW_C_HEADERS define to control if the MSL C headers are used or not        cc
        *011115 Added new flags _MSL_USE_INLINE & _MSL_INLINE        JWW 011126 Added
        *_MSL_MALLOC_IS_ALTIVEC_ALIGNED to know if malloc returns AltiVec aligned memory        cc  011128
        *Added _MSL_TLS macro and made _MSL_CDECL follow ifndef layout to remove dest os        cc  011219
        *Added C++ check to msl use inline        JWW 020308 New _MSL_DO_NOT_INLINE macro for defining how to
        *place functions in the library        JWW 020404 New _MSL_ASSERT_DISPLAYS_FUNC macro for defining if
        *assert() displays __func__        PMK 020904 Added _MSL_LITTLE_ENDIAN flag, used this flag in
        *_strtold() function        JWW 020501 Moved _MSL_ASSERT_DISPLAYS_FUNC definition to cassert so it can
        *be redefined easier        cc  020531 Removed the obsolete __tls macro and the comment that it was to
        *be removed in Pro 9        JWW 020604 Added _MSL_LOCALDATA macro definition to make multithread code
        *easier to read        JWW 020701 Always use compiler inlining option to define _MSL_USE_INLINE, don't
        *special case C++        JWW 020917 Added _MSL_RESTRICT to get the restrict type specifier for certain
        *C99 functions        ejs 020924 Fix _MSL_LITTLE_ENDIAN setting        cc  021001 Added support for
        *_MSL_C_LOCALE_ONLY & _MSL_STRERROR_KNOWS_ERROR_NAMES        JWW 021010 Added wchar_t file I/O
        *routines controlled by _MSL_WFILEIO_AVAILABLE        mm  030218 Changed _MSL_C9X_ to _MSL_C99 in
        *wrapper name        JWW 030224 Generate an error when the _MSL_C99 flag and the compiler C99 setting
        *is incompatible        JWW 030224 Changed __MSL_LONGLONG_SUPPORT__ flag into the new more
        *configurable _MSL_LONGLONG        JWW 030224 Changed __NO_WIDE_CHAR flag into the new more
        *configurable _MSL_WIDE_CHAR        JWW 030321 Added __has_intrinsic macro to determine the most
        *efficient way to perform operations        ejs 030613 Remove __has_intrinsic overrides now that
        *compiler supports them        cc  030808 Changed _MSL_NO_FLOAT_HEX to _MSL_FLOAT_HEX and turned it on
        * cc  031201 Added _MSL_PTHREADS flag
        * cc  040204 The _No_Time_OS_Support flag has been deprecated and replaced with
        *_MSL_OS_TIME_SUPPORT        cc  040217 The _No_Floating_Point flag has been deprecated and replaced
        *with _MSL_FLOATING_POINT        cc  040217 Changed _No_Disk_File_OS_Support to
        *_MSL_OS_DISK_FILE_SUPPORT        JWW 040401 Changed _No_Alloc_OS_Support to _MSL_OS_ALLOC_SUPPORT        JWW
        *040405 Changed _Unbuffered_Console to _MSL_BUFFERED_CONSOLE        cc  010811 Removed need to turn on
        *_MSL_C_LOCALE_ONLY when _MSL_C99 is off        JWW 050128 When using MSL C++ on top of a third party
        *C (non-MSL), put C items into std namespace        cc  050518 Removed _MSL_ASSERT_DISPLAYS_FUNC flag
        *since cassert does the configuring        bn  060605 Added _MSL_COMPLEX
        */
